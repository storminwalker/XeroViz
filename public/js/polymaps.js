...
po.fullscreen = function() {
  var fullscreen = {};
  var svg = Ext.get(po.svg("svg"));
  var circle = Ext.get(po.svg("circle"));
  var arrow = Ext.get(po.svg("path"));
  var map;
  var container;
  var isFullscreen = 0;
  ...
  svg.on("click", goFullscreen);

  Ext.getBody().on("keydown", function(e){
    e.keyCode == 27 && isFullscreen && goFullscreen();
  });
  ...
  //circle.appendChild("<title>Toggle Fullscreen. (ESC)</title>");

  arrow.set({
  	"d": "M0,0L0,.5 2,.5 2,1.5 4,0 2,-1.5 2,-.5 0,-.5Z",
  	"pointer-events": "none",
    "fill": "#aaa",
    "transform": "translate(16,16)rotate(-45)scale(5)translate(-1.85,0)",
  }, true);
  ...
  fullscreen.map = function(x) {
    ...
    if (map = x) {
      container = Ext.get(map.container());
      draw();
    }
  ...
  function goFullscreen() {
    if(isFullscreen = !isFullscreen) {
      ...
      arrow.set({
      	"transform": "translate(16,16) rotate(135) scale(5) translate(-1.85,0)"
      });
  ...
  function draw() {
    container.parent().appendChild(svg);
